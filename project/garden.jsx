...
// Build a smooth cubic-bezier path through the poem nodes.
...
// Wrap a title string into lines of max `maxLen` chars at word boundaries
      ...
      <div className="reader-panel" onClick={e => e.stopPropagation()} ref={ref}
        ...
        <div className="reader-inner">
          <div className="reader-motif">
            {poem.image ? (
              <img src={poem.image} alt="" style={{ width: 120, height: 120, borderRadius: "50%", objectFit: "cover", boxShadow: "0 2px 18px rgba(60,40,10,0.18)" }} />
            ) : (
...
function Garden() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // Progress: how far has the viewport center traveled down the SVG?
      const svgTop = rect.top;
      const svgH = rect.height;

      // Reveal each node precisely when its screen position enters the viewport
      ...
      // Thread progress: drive from first node's y to last node's y (not full SVG height)
  ...
  // Compute thread head position for the moving orb
  let threadHead = null;
  ...
  return (
    <>
      ...
      {/* The garden canvas — inline, scrollable */}
